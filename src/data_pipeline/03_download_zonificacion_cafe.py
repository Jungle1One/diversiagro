"""
=============================================================================
Dataset 3: Zonificación de aptitud para el cultivo de café
Fuente: UPRA via datos.gov.co (Socrata API)
ID: kwvf-nwea
Total registros en origen: 508,129 polígonos
Valle del Cauca: ~15,981 polígonos
=============================================================================

Este script:
1. Descarga datos de zonificación de café para Valle del Cauca
...
# CONFIGURACIÓN
# =============================================================================
BASE_URL = "https://www.datos.gov.co/api/v3/views/kwvf-nwea/query.json"
...
def download_data():
    """
    Descarga datos de zonificación de café para Valle del Cauca.
    Usa API v3 con POST y paginación.
    """
    log("=" * 60)
    log("PASO 1: DESCARGA DE DATOS DE ZONIFICACIÓN DE CAFÉ")
    ...
    raw_file = os.path.join(DATA_RAW_DIR, "zonificacion_cafe_valle_raw.csv")
...
def explore_data(df):
    ...
    report = []
    report.append("# Reporte de Exploración – Zonificación de Café (UPRA)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"\n## Información General")
    report.append(f"- **Fuente**: UPRA – datos.gov.co (ID: kwvf-nwea)")
    ...
    # Guardar reporte
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "05_cafe_zonificacion_exploration.md")
...
def clean_and_aggregate(df):
    ...
    if "the_geom" in df.columns:
        try:
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cafe_valle_limpio.geojson")
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cafe_valle_limpio.csv")
    ...
    area_pivot["indice_aptitud_cafe"] = (numerador / denominador).round(3)
    
    # Guardar
    muni_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cafe_valle_municipio.csv")
...
def generate_quality_report(df, area_pivot):
    ...
    report = []
    report.append("# Reporte de Calidad – Zonificación de Café (Procesado)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    report.append(f"|---------|-----------|-------------|")
    report.append(f"| zonificacion_cafe_valle_limpio.csv | {len(df):,} | Polígonos con aptitud y área |")
    report.append(f"| zonificacion_cafe_valle_municipio.csv | {len(area_pivot)} | Resumen por municipio: % aptitud |")
    ...
    report.append(f"|-----------|---------------|-------------------|-----------------|")
    sorted_munis = area_pivot.sort_values("indice_aptitud_cafe", ascending=False)
    for _, row in sorted_munis.iterrows():
        report.append(f"| {row['municipio']} | {row['indice_aptitud_cafe']:.3f} | {row['aptitud_dominante']} | {row['area_total_ha']:,.1f} |")
    
    report.append(f"\n## Evaluación de Utilidad para el Modelo RFRK")
    report.append(f"\n### ✅ Fortalezas")
    report.append(f"- Clasificación oficial de aptitud a escala 1:100.000")
    report.append(f"- Cubre todos los municipios del Valle del Cauca")
    ...
    report.append(f"- No incluye variables biofísicas detalladas (temperatura, altitud específica)")
    report.append(f"- Geometrías excluidas de la descarga tabular")
    report.append(f"\n### 🎯 Conclusión")
    report.append(f"- **¿Es útil?**: ✅ SÍ")
    report.append(f"- **Uso principal**: Predictor base de aptitud agroecológica para café")
    ...
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "06_cafe_zonificacion_quality.md")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(report_text)
...
if __name__ == "__main__":
    log("🚀 Iniciando procesamiento de Zonificación de Café")
    log(f"   Departamento: {DEPARTAMENTO_INTERES}")
    log("")
    ...
    log(f"   - data/raw/zonificacion_cafe_valle_raw.csv")
    log(f"   - data/processed/zonificacion_cafe_valle_limpio.csv")
    log(f"   - data/processed/zonificacion_cafe_valle_municipio.csv")
    log(f"   - data/reports/05_cafe_zonificacion_exploration.md")
    log(f"   - data/reports/06_cafe_zonificacion_quality.md")
