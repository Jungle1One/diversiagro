"""
=============================================================================
Dataset 7: Zonificación de aptitud para el cultivo comercial de papaya híbrido Tainung
Fuente: UPRA via datos.gov.co (Socrata API)
ID: urxm-qzje
Total registros en origen: ~199,248 polígonos
Valle del Cauca: ~7,634 polígonos
=============================================================================

Este script:
1. Descarga datos de zonificación de papaya para Valle del Cauca
...
# CONFIGURACIÓN
# =============================================================================
BASE_URL = "https://www.datos.gov.co/api/v3/views/urxm-qzje/query.json"
...
def download_data():
    """
    Descarga datos de zonificación de papaya para Valle del Cauca.
    Usa API v3 con POST y paginación.
    """
    log("=" * 60)
    log("PASO 1: DESCARGA DE DATOS DE ZONIFICACIÓN DE PAPAYA")
    ...
    raw_file = os.path.join(DATA_RAW_DIR, "zonificacion_papaya_valle_raw.csv")
...
def explore_data(df):
    ...
    report = []
    report.append("# Reporte de Exploración – Zonificación de Papaya (UPRA)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"\n## Información General")
    report.append(f"- **Fuente**: UPRA – datos.gov.co (ID: urxm-qzje)")
    ...
    # Guardar reporte
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "13_papaya_zonificacion_exploration.md")
...
def clean_and_aggregate(df):
    ...
    if "the_geom" in df.columns:
        try:
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_papaya_valle_limpio.geojson")
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_papaya_valle_limpio.csv")
    ...
    area_pivot["indice_aptitud_papaya"] = (numerador / denominador).round(3)
    
    # Guardar
    muni_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_papaya_valle_municipio.csv")
...
def generate_quality_report(df, area_pivot):
    ...
    report = []
    report.append("# Reporte de Calidad – Zonificación de Papaya (Procesado)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    report.append(f"|---------|-----------|-------------|")
    report.append(f"| zonificacion_papaya_valle_limpio.csv | {len(df):,} | Polígonos con aptitud y área |")
    report.append(f"| zonificacion_papaya_valle_municipio.csv | {len(area_pivot)} | Resumen por municipio: % aptitud |")
    ...
    report.append(f"|-----------|---------------|-------------------|-----------------|")
    sorted_munis = area_pivot.sort_values("indice_aptitud_papaya", ascending=False)
    for _, row in sorted_munis.iterrows():
        report.append(f"| {row['municipio']} | {row['indice_aptitud_papaya']:.3f} | {row['aptitud_dominante']} | {row['area_total_ha']:,.1f} |")
    ...
    report.append(f"- **Uso principal**: Predictor base de aptitud agroecológica para papaya")
    ...
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "14_papaya_zonificacion_quality.md")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(report_text)
...
if __name__ == "__main__":
    log("🚀 Iniciando procesamiento de Zonificación de Papaya")
    log(f"   Departamento: {DEPARTAMENTO_INTERES}")
    log("")
    ...
    log(f"   - data/raw/zonificacion_papaya_valle_raw.csv")
    log(f"   - data/processed/zonificacion_papaya_valle_limpio.csv")
    log(f"   - data/processed/zonificacion_papaya_valle_municipio.csv")
    log(f"   - data/reports/13_papaya_zonificacion_exploration.md")
    log(f"   - data/reports/14_papaya_zonificacion_quality.md")
