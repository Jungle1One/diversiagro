"""
=============================================================================
Dataset 6: Zonificación de aptitud para el cultivo comercial de caña panelera
Fuente: UPRA via datos.gov.co (Socrata API)
ID: p9xp-sm4v
Total registros en origen: ~128,852 polígonos
Valle del Cauca: ~5,080 polígonos
=============================================================================

Este script:
1. Descarga datos de zonificación de caña panelera para Valle del Cauca
...
# CONFIGURACIÓN
# =============================================================================
BASE_URL = "https://www.datos.gov.co/api/v3/views/p9xp-sm4v/query.json"
...
def download_data():
    """
    Descarga datos de zonificación de caña panelera para Valle del Cauca.
    Usa API v3 con POST y paginación.
    """
    log("=" * 60)
    log("PASO 1: DESCARGA DE DATOS DE ZONIFICACIÓN DE CAÑA PANELERA")
    ...
    raw_file = os.path.join(DATA_RAW_DIR, "zonificacion_cana_panelera_valle_raw.csv")
...
def explore_data(df):
    ...
    report = []
    report.append("# Reporte de Exploración – Zonificación de Caña Panelera (UPRA)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append(f"\n## Información General")
    report.append(f"- **Fuente**: UPRA – datos.gov.co (ID: p9xp-sm4v)")
    ...
    # Guardar reporte
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "11_cana_panelera_zonificacion_exploration.md")
...
def clean_and_aggregate(df):
    ...
    if "the_geom" in df.columns:
        try:
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cana_panelera_valle_limpio.geojson")
            ...
            clean_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cana_panelera_valle_limpio.csv")
    ...
    area_pivot["indice_aptitud_cana"] = (numerador / denominador).round(3)
    
    # Guardar
    muni_file = os.path.join(DATA_PROCESSED_DIR, "zonificacion_cana_panelera_valle_municipio.csv")
...
def generate_quality_report(df, area_pivot):
    ...
    report = []
    report.append("# Reporte de Calidad – Zonificación de Caña Panelera (Procesado)")
    report.append(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    report.append(f"|---------|-----------|-------------|")
    report.append(f"| zonificacion_cana_panelera_valle_limpio.csv | {len(df):,} | Polígonos con aptitud y área |")
    report.append(f"| zonificacion_cana_panelera_valle_municipio.csv | {len(area_pivot)} | Resumen por municipio: % aptitud |")
    ...
    report.append(f"|-----------|---------------|-------------------|-----------------|")
    sorted_munis = area_pivot.sort_values("indice_aptitud_cana", ascending=False)
    for _, row in sorted_munis.iterrows():
        report.append(f"| {row['municipio']} | {row['indice_aptitud_cana']:.3f} | {row['aptitud_dominante']} | {row['area_total_ha']:,.1f} |")
    ...
    report.append(f"- **Uso principal**: Predictor base de aptitud agroecológica para caña panelera")
    ...
    report_text = "\n".join(report)
    report_file = os.path.join(DATA_REPORTS_DIR, "12_cana_panelera_zonificacion_quality.md")
    with open(report_file, "w", encoding="utf-8") as f:
        f.write(report_text)
...
if __name__ == "__main__":
    log("🚀 Iniciando procesamiento de Zonificación de Caña Panelera")
    log(f"   Departamento: {DEPARTAMENTO_INTERES}")
    log("")
    ...
    log(f"   - data/raw/zonificacion_cana_panelera_valle_raw.csv")
    log(f"   - data/processed/zonificacion_cana_panelera_valle_limpio.csv")
    log(f"   - data/processed/zonificacion_cana_panelera_valle_municipio.csv")
    log(f"   - data/reports/11_cana_panelera_zonificacion_exploration.md")
    log(f"   - data/reports/12_cana_panelera_zonificacion_quality.md")
